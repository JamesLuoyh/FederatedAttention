...
class StackoverflowNWPClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        return self.get_parameters(), count, {'loss': float(loss), 'gradient_alpha': float(0.0), 'gradient_personalized_objective': float(0.0)}
...
class EMNISTClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        test_data_ = test_data.create_tf_dataset_for_client(self.cid)
        self.set_parameters(parameters)
        loss, count, max_epoch = emnist_train(self.net, train_data_, test_data_)
        print('b')
