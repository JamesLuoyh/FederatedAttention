...
def get_federated_datasets(
  ...
  (stackoverflow_train, _,
   stackoverflow_test) = tff.simulation.datasets.stackoverflow.load_data(cache_dir='./dataset_cache')
...
def get_centralized_datasets(
  ...
  raw_train, raw_validation, raw_test = (
      tff.simulation.datasets.stackoverflow.load_data(cache_dir='./dataset_cache'))
